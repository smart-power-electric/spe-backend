...
import { ILogger, LogMode, AWSCloudWatchConfig } from './logger.interface';
import { LogLevel, LogLevelType, Transport } from './logger.entity';
...
export class LoggingService implements ILogger {
  ...
  static getAWSCloudWatchConfig(): AWSCloudWatchConfig {
    const isEnvIncomplete =
      !process.env.AWS_REGION ||
      !process.env.AWS_BACKEND_ACCESS_KEY_ID ||
      !process.env.AWS_BACKEND_SECRET_ACCESS_KEY ||
      !process.env.AWS_CW_LOG_GROUP ||
      !process.env.AWS_CW_LOG_STREAM;
    const loggerMode: LogMode = isEnvIncomplete ? 'LOCAL' : 'SERVER';

    return {
      region: process.env.AWS_REGION ?? 'us-east-1',
      awsKey: process.env.AWS_BACKEND_ACCESS_KEY_ID ?? 'aws_access_key_id',
      awsSecret: process.env.AWS_BACKEND_SECRET_ACCESS_KEY ?? 'aws_secret_access_key',
      logGroup: process.env.AWS_CW_LOG_GROUP ?? 'log_group',
      logStream: process.env.AWS_CW_LOG_STREAM ?? 'log_stream',
      MODE: loggerMode,
    };
  }
   ...
   * @param {Context} [context] - Optional context information.
   */
  public verbose(context: Context | null, message: unknown, ...arg2: unknown[]) {
  ...
  private log(
    ...
    const elapsed = context ? new Date().getTime() - context.startTime.getTime() : null;
    const requestId = context ? context.requestId : null;
    logFunction(`${logLine}${argLine}`, { requestId: requestId, elapsedTime: elapsed });
  }

...
export const sortOrderDictionary = Object.fromEntries(
  sortOrderList.map((key) => [key, key]),
) as {
...
export const optionalNullableStringSchema = optionalStringSchema
  .nullable()
  .default(null);

export const optionalStringToBooleanSchema = optionalStringSchema.transform(
  (value) => {
    if (value === 'true') {
      return true;
    }

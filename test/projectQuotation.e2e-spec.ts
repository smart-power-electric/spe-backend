...
describe('ProjectQuotationModule (e2e)', () => {
  const jestConsole = console;
  let app: INestApplication;
  let dbContainer: StartedPostgreSqlContainer;
  let appPort = 0;
  let api: ProjectQuotationApi;
  let baseStage: Stage;
  ...
  beforeAll(async () => {
    ...
    api = new ProjectQuotationApi(config);
  ...
  it('POST /project-quotation', async () => {
    const newItem: CreateProjectQuotationRequest = {
      projectId: baseProject.id ?? '',
      materialId: baseMaterial.id ?? '',
      serviceId: baseService.id ?? '',
      quantity: faker.number.int({ min: 0, max: 1000 }),
      totalCost: faker.number.int({ min: 0, max: 1000 }),
    };
    const item = await api.createProjectQuotation({
      createProjectQuotationRequest: newItem,
    });
    ...
    expect(item.materialId).toEqual(newItem.materialId);
    expect(item.serviceId).toEqual(newItem.serviceId);
    expect(item.quantity).toEqual(newItem.quantity);
    expect(item.totalCost).toEqual(newItem.totalCost);
    expect(item.createdAt).toBeDefined();
    expect(item.updatedAt).toBeNull();
  }, 600000);
  it('GET /project-quotation', async () => {
    const newItem: CreateProjectQuotationRequest = {
      projectId: baseProject.id ?? '',
      materialId: baseMaterial.id ?? '',
      serviceId: baseService.id ?? '',
    ...
    const result = await api.findAllProjectQuotation({});
    expect(result).toBeDefined();
    expect(result.data).toBeDefined();
    expect(result.total).toBeGreaterThan(0);
    expect(result.data?.length).toEqual(result.total);

    const result2 = await api.findAllProjectQuotation({
    ...
    const result3 = await api.findAllProjectQuotation({
    ...
    expect(
      result3.data.find((x) =>
        x.serviceId
          ?.toLowerCase()
          .includes(newItem.serviceId?.toLowerCase() ?? ''),
      ),
    ).toBeDefined();

    const result5 = await api.findAllProjectQuotation({ limit: 0 });
    expect(result5).toBeDefined();
    expect(result5.data).toBeDefined();
    expect(result5.total).toBeGreaterThan(0);
    expect(result5.data?.length).toEqual(0);

    const result6 = await api.findAllProjectQuotation({ offset: 1000 });
  ...
  it('GET /project-quotation/:id', async () => {
    const newItem: CreateProjectQuotationRequest = {
      projectId: baseProject.id ?? '',
      materialId: baseMaterial.id ?? '',
      serviceId: baseService.id ?? '',
    ...
    const result = await api.findOneProjectQuotation({ id: item.id ?? '' });
    expect(result).toBeDefined();
    expect(result.id).toEqual(item.id);
  }, 600000);

  it('PATCH /project-quotation/:id', async () => {
    const newItem: CreateProjectQuotationRequest = {
      projectId: baseProject.id ?? '',
      materialId: baseMaterial.id ?? '',
      serviceId: baseService.id ?? '',
    ...
    const updatedClient: UpdateProjectQuotationRequest = {
      quantity: faker.number.int({ min: 0, max: 1000 }),
      totalCost: faker.number.int({ min: 0, max: 1000 }),
    };
    const result = await api.updateProjectQuotation({
      id: item.id ?? '',
      updateProjectQuotationRequest: updatedClient,
    });

    expect(result).toBeDefined();
    expect(result.id).toEqual(item.id);
    expect(result.quantity).toEqual(updatedClient.quantity);
    expect(result.totalCost).toEqual(updatedClient.totalCost);
    expect(result.createdAt).toBeDefined();
    expect(result.updatedAt).toBeDefined();
  });

  it('DELETE /project-quotation/:id', async () => {
    const newItem: CreateProjectQuotationRequest = {
      projectId: baseProject.id ?? '',
      materialId: baseMaterial.id ?? '',
      serviceId: baseService.id ?? '',
    ...
    try {
      await api.findOneProjectQuotation({ id: item.id ?? '' });
    } catch (error) {

...
describe('ServiceModule (e2e)', () => {
  ...
  it('POST /service', async () => {
    const newItem: CreateServiceRequest = {
      name: faker.person.fullName(),
      description: faker.lorem.words(),
      unitCost: faker.number.int({ min: 0, max: 1000 }),
    };
    const item = await api.createService({
      createServiceRequest: newItem,
    });
  ...
  it('GET /service', async () => {
    ...
    const result = await api.findAllService({});
    expect(result).toBeDefined();
    expect(result.data).toBeDefined();
    expect(result.total).toBeGreaterThan(0);
    expect(result.data?.length).toEqual(result.total);

    const result2 = await api.findAllService({
      name: newItem.name ?? '',
    });
  ...
  it('GET /service/:id', async () => {
    ...
    const result = await api.findOneService({ id: item.id ?? '' });
    expect(result).toBeDefined();
  ...
  it('PATCH /service/:id', async () => {
    ...
    const updatedClient: UpdateServiceRequest = {
      name: faker.person.fullName(),
      description: faker.lorem.words(),
      unitCost: faker.number.int({ min: 0, max: 1000 }),
    };
    const result = await api.updateService({
      id: item.id ?? '',
      updateServiceRequest: updatedClient,
    });
  ...
  it('DELETE /service/:id', async () => {
    ...
    try {
      await api.findOneService({ id: item.id ?? '' });
    } catch (error) {

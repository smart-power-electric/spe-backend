...
describe('StageModule (e2e)', () => {
  ...
  it('POST /stage', async () => {
    const newItem: CreateStageRequest = {
      name: faker.person.fullName(),
      description: faker.lorem.words(),
      projectId: baseProject.id,
      percentage: faker.number.int({ min: 0, max: 100 }),
    ...
    const item = await api.createStage({
      createStageRequest: newItem,
    });
    ...
    expect(item.percentage).toEqual(newItem.percentage);
    expect(item.adjustedPercentage).toEqual(newItem.adjustedPercentage);
  ...
  it('GET /stage', async () => {
    ...
    const result = await api.findAllStage({});
    expect(result).toBeDefined();
    expect(result.data).toBeDefined();
    expect(result.total).toBeGreaterThan(0);
    expect(result.data?.length).toEqual(result.total);

    const result2 = await api.findAllStage({
      projectId: baseProject.id,
    });
    ...
    const result3 = await api.findAllStage({
      name: newItem.name ?? '',
    });
  ...
  it('GET /stage/:id', async () => {
    ...
    const result = await api.findOneStage({ id: item.id ?? '' });
    expect(result).toBeDefined();
  ...
  it('PATCH /stage/:id', async () => {
    ...
    const updatedClient: UpdateStageRequest = {
      name: faker.person.fullName(),
      description: faker.lorem.words(),
      percentage: faker.number.int({ min: 0, max: 100 }),
      adjustedPercentage: faker.number.int({ min: 0, max: 100 }),
    ...
    const result = await api.updateStage({
      id: item.id ?? '',
      updateStageRequest: updatedClient,
    });
    ...
    expect(result.percentage).toEqual(updatedClient.percentage);
    expect(result.adjustedPercentage).toEqual(updatedClient.adjustedPercentage);
  ...
  it('DELETE /stage/:id', async () => {
    ...
    try {
      await api.findOneStage({ id: item.id ?? '' });
    } catch (error) {
